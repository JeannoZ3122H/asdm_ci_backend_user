...
@Component({
  selector: 'app-asigned-projet-non-commercial-evaluateur',
  templateUrl: './asigned-projet-non-commercial-evaluateur.component.html',
  styleUrls: ['./asigned-projet-non-commercial-evaluateur.component.css']
})
export class AsignedProjetNonCommercialEvaluateurComponent implements OnInit {
    ...
    constructor(
        ...
        private _dialogRef: MatDialogRef<AsignedProjetNonCommercialEvaluateurComponent>,
    ){
    }
    ...
    ngOnInit(){
        if(this.data != null) {
            ...
            let niveau_code: any = localStorage.getItem('niveau_code_non_commercial');
            ...
            if(this.data.type_asigned == 'asigned'){
                ...
                let check_old_action: any = localStorage.getItem('old_code_project_assigned_non_commercial');
                ...
                if(check_old_action == null){
                    ...
                    this.getListeEvaluateur();
                    let old_code_project_assigned: any = localStorage.getItem('old_code_project_assigned_non_commercial');
                    if(old_code_project_assigned != null && old_code_project_assigned == this.data.project_code){
                        this.getOldAssignedVar();
                    }else{
                        localStorage.removeItem('old_code_project_assigned_non_commercial');
                        localStorage.removeItem('step_end_non_commercial');
                        setTimeout(() => {
                            this.initAssignedVar();
                        }, 1500);
                    }
...
// 😇 *** ASSIGNED
    ...
    asignedProjetAsEvaluateur() {
        ...
        this._loading.start();
        this._request.asignedProjetNonCommercial(data).subscribe(
            {
                next: (response: any) => {
                    if (response.code == 200) {
                        localStorage.setItem('step_end_non_commercial', '1');
                        if(this.current_niv < 2){
                            localStorage.setItem('niveau_code_non_commercial', this._liste_niveau[this.current_niv].niveau_code);
                        }else{
                            this.is_end = 2;
                        }
                        this.getListeEvaluateur();
...
// 😇 *** REASSIGNED
    ...
    reasignedProjetAsEvaluateur() {
        ...
        this._request.reasignedProjetNonCommercial(data.project_code, data.old_instructeur_id, data.instructeur_id, data.date_fin).subscribe(
            {
                next: (response: any) => {
                    if (response.code == 200) {
                        setTimeout(() => {
...
// 😇 *** ASSIGNED
    ...
    initAssignedVar(){
        this.current_niv = 1;
        localStorage.setItem('old_code_project_assigned_non_commercial', this.data.project_code);
        localStorage.setItem('step_end_non_commercial', '0');
    }
    // get variable for old assigned
    getOldAssignedVar(){
        let current_niv: any = localStorage.getItem('step_end_non_commercial');
        if(current_niv != null){
    ...
    removeOldDataToLs(){
        localStorage.removeItem('old_code_project_assigned_non_commercial');
        localStorage.removeItem('step_end_non_commercial');
        localStorage.removeItem('niveau_code_non_commercial');
    }
    // 😇😇 ************************************************ //
// ****************** END FUNCTION EVENTS ****************** //
// ************************************************ 😇😇😇😇 //
