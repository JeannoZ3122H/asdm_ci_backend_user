...
@Component({
  selector: 'app-second-evaluator-details-projet-help',
  templateUrl: './second-evaluator-details-projet-help.component.html',
  styleUrls: ['./second-evaluator-details-projet-help.component.css']
})
export class SecondEvaluatorDetailsProjetHelpComponent implements OnInit{
...
// ****************** START FUNCTION REQUEST TO API ****************** //
    ...
    // 😇😇 **** STORE DATA
        // 😇 SET SINGLE DATA COMMENT
            ...
            setSingleCommentData(data: any, element: string){
                ...
                const dialogRef = this._dialog.open(SetCommentsHelpByElementComponent,
                {
                    width: 'auto',
                    height: 'auto',
                    data: _data
        ...
        // 😇 UPDATE SINGLE DATA COMMENT
            ...
            updateSingleCommentData(data: any, element: string, element_comments: any){
                ...
                const dialogRef = this._dialog.open(UpdateCommentsHelpByElementComponent,
                {
                    width: 'auto',
                    height: 'auto',
                    data: _data
    ...
    // 😇😇 **** STORE EVALUATION BY ETUDE
        ...
        setOrUpdateEtudeProjectDataToDB(){
            ...
            this._loading.start();
            this._request_ev.setOrUpdateEtudeProjectHelpDataToDB(data).subscribe(
                {
                    next: (response: any) => {
        ...
        setOrUpdateDecisionProjectDataToDB(){
            ...
            this._loading.start();
            this._request_ev.setOrUpdateDecisionProjectHelpDataToDB(data).subscribe(
                {
                    next: (response: any) => {
                        if (response.code == 200) {
                            this.removeLs();
