...
@Component({
  selector: 'app-second-evaluaor-list-projet-resolved-commercial',
  templateUrl: './second-evaluaor-list-projet-resolved-commercial.component.html',
  styleUrls: ['./second-evaluaor-list-projet-resolved-commercial.component.css']
})
export class SecondEvaluaorListProjetResolvedCommercialComponent implements OnInit{
    ...
    ngOnInit() {
       this.evaluator = this._localeStorage.getDataToStorage();
        this.getListeProjetCommercialResolved();
    }

    getListeProjetCommercialResolved() {
        this._request.getListeProjetCommercialResolved(this.evaluator.id).subscribe(
            {
                next: (response: any) => {
